...
const replaced = data.replace(/= ?3000/g, '=3001').replace(/= ?3e3/g, '=3001');
fs.writeFileSync('dist/server-test.cjs', replaced);

import child_process from 'child_process';
const cp = child_process.spawn('npx', ['cross-env', 'PORT=3001', 'NODE_ENV=production', 'node', 'dist/server-test.cjs']);
...
cp.stdout.on('data', d => {
  if (d.toString().includes('Server running on http://localhost:3001')) {
     isServerUp = true;
